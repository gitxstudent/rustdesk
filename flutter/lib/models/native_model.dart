...
class PlatformFFI {
  ...
  VNFapImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libvnfap.so')
        : isLinux
            ? DynamicLibrary.open('libvnfap.so')
            : isWindows
                ? DynamicLibrary.open('libvnfap.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibvnfap.dylib") :
  ...
  void _startListenEvent(VNFapImpl vnfapImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = vnfapImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
