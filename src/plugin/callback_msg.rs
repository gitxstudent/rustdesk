...
fn handle_msg_to_vnfap(id: String, content: *const c_void, len: usize) -> PluginReturn {
    let s = early_return_value!(
        std::str::from_utf8(unsafe { std::slice::from_raw_parts(content as _, len) }),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg string"
    );
    let msg_to_vnfap = early_return_value!(
        serde_json::from_str::<MsgToVNFap>(s),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg '{}'",
        s
    );
...
fn request_plugin_sign(id: String, msg_to_vnfap: MsgToVNFap) -> PluginReturn {
    let signature_data = early_return_value!(
        std::str::from_utf8(&msg_to_vnfap.data),
        ERR_CALLBACK_INVALID_MSG,
