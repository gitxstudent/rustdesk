...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "vnfap".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/vnfap-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "VNFap wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "VNFap wants to uninstall the plugin",
    )
}
