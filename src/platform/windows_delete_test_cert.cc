...
//*************************************************************
//
//  DeleteVNFapTestCertsW_SingleHive()
//
//  Purpose:    Deletes VNFap Test certificates and wrong key stores
//
...
BOOL DeleteVNFapTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteVNFapTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteVNFapTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteVNFapTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteVNFapTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
